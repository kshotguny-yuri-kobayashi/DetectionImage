...
def main():
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],4) * 100  # 猫の確率(%)
        pred1 = round(prediction[1],4) * 100  # 犬の確率(%)
        pred2 = round(prediction[2],4) * 100  # アザラシの確率(%)
        pred3 = round(prediction[3],3) * 100  #フクロモモンガの確率(%)
        pred4 = round(prediction[4],3) * 100  #目玉の親父の確率(%)
        ...
        if classNo == 0:
            ...
            st.subheader(f"これは{pred3}％の確率で「フクロモモンガ」です！")
        elif classNo == 4:
            st.subheader(f"これは{pred4}％の確率で「目玉の親父」です！")
